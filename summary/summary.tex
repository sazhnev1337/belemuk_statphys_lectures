...
\textbf{Большой канонический ансамбль} (фиксированы $T$ и $\mu$). Вероятность состояния $(k, N)$:
\begin{equation*}
    w_{k,N} = \frac{1}{\mathcal{Z}}\,e^{(\mu N - E_{k,N})/T}
\end{equation*}
\textbf{Большая статистическая сумма:}
\begin{equation*}
    \mathcal{Z} = \sum_{N=0}^{\infty}\sum_k e^{(\mu N - E_{k,N})/T} = \mathrm{Sp}\!\left(e^{(\mu\hat{N}-\hat{H})/T}\right)
\end{equation*}
\textbf{Большой потенциал:} $\Omega = -T\ln\mathcal{Z} = -PV$; $\;d\Omega = -S\,dT - P\,dV - N\,d\mu$
\begin{equation*}
    S = -\!\left(\tfrac{\partial\Omega}{\partial T}\right)_{\!V,\mu},\quad
    P = -\!\left(\tfrac{\partial\Omega}{\partial V}\right)_{\!T,\mu}
\end{equation*}
\begin{equation*}
    N = -\!\left(\tfrac{\partial\Omega}{\partial\mu}\right)_{\!T,V} = T\tfrac{\partial\ln\mathcal{Z}}{\partial\mu}
\end{equation*}

\textbf{Уравнение Гиббса--Дюгема.} Так как $\Omega = -PV$, то $d\Omega = -PdV - VdP$. Сравнивая с $d\Omega = -SdT - PdV - Nd\mu$, получаем:
\begin{equation*}
    VdP = SdT + Nd\mu \;\Longrightarrow\; dP = s\,dT + n\,d\mu
\end{equation*}
где $s = S/V$ --- плотность энтропии, $n = N/V$ --- концентрация. Следствия:
\begin{equation*}
    \left(\tfrac{\partial P}{\partial T}\right)_{\!\mu} = \tfrac{S}{V}, \qquad
    \left(\tfrac{\partial P}{\partial \mu}\right)_{\!T} = \tfrac{N}{V}
\end{equation*}
Переменные $T, \mu, P$ \emph{не независимы} (соотношение Гиббса--Дюгема).

\textbf{Потенциал Гиббса и $\mu$.} Экстенсивность: $\Phi(T,P,N) = \mu(T,P)\,N$. Из $d\Phi = -SdT + VdP + \mu dN$ и $d(\mu N) = \mu dN + Nd\mu$ следует $Nd\mu = -SdT + VdP$, откуда:
\begin{equation*}
    \left(\tfrac{\partial\mu}{\partial T}\right)_{\!P} = -\tfrac{S}{N}, \qquad
    \left(\tfrac{\partial\mu}{\partial P}\right)_{\!T} = \tfrac{V}{N} = v
\end{equation*}

\textbf{Флуктуации числа частиц.} Применяя $T\partial_\mu$ дважды к $\ln\mathcal{Z}$:
\begin{equation*}
    \langle\Delta N^2\rangle = T\frac{\partial N}{\partial\mu}, \qquad
    \frac{\sqrt{\langle\Delta N^2\rangle}}{N} \sim \frac{1}{\sqrt{N}} \to 0
\end{equation*}
Относительные флуктуации $N$ убывают как $1/\sqrt{N}$: большой и канонический ансамбли дают одинаковые средние.
...
\textbf{Гамма-функция} (нужна для вычисления интегралов по состояниям):
\begin{equation*}
    \Gamma(z) = \int_0^\infty t^{z-1}e^{-t}\,dt, \qquad \Gamma(z+1) = z\,\Gamma(z)
\end{equation*}
Целые: $\Gamma(n+1)=n!$. Полуцелые: $\Gamma(\tfrac{1}{2})=\sqrt{\pi}$, откуда
\begin{equation*}
...
\end{equation*}
Применение: $\displaystyle\int_0^\infty \varepsilon^s e^{-\varepsilon/T}d\varepsilon = T^{s+1}\Gamma(s+1)$; при $s=\tfrac{1}{2}$ (3D газ) даёт $T^{3/2}\tfrac{\sqrt{\pi}}{2}$.

\textbf{Плотность состояний} идеального 3D-газа (фактор спинового вырождения $g$):
\begin{equation*}
    \nu(\varepsilon) = \frac{gVm^{3/2}}{\sqrt{2}\,\pi^2\hbar^3}\sqrt{\varepsilon} \equiv A\sqrt{\varepsilon}
\end{equation*}
Термодинамические средние через $\nu(\varepsilon)$:
\begin{equation*}
    \langle N\rangle = \int_0^\infty \nu(\varepsilon)\,\bar{n}_\varepsilon\,d\varepsilon, \qquad
    \langle E\rangle = \int_0^\infty \varepsilon\,\nu(\varepsilon)\,\bar{n}_\varepsilon\,d\varepsilon
\end{equation*}

\textbf{Ультраквантовый предел фермионов ($T=0$).} Функция ФД вырождается в ступеньку Хевисайда: $\bar{n}_\varepsilon = \theta(\varepsilon_F - \varepsilon)$, где $\varepsilon_F \equiv \mu(T{=}0)$ --- \emph{энергия Ферми}. Все состояния с $|\mathbf{p}| \leq p_F$ заполнены:
\begin{equation*}
    N = \frac{gV}{(2\pi\hbar)^3}\cdot\frac{4\pi}{3}p_F^3
    \;\Rightarrow\;
    p_F = \hbar\!\left(\frac{6\pi^2 n}{g}\right)^{\!1/3}
\end{equation*}
\begin{equation*}
    \varepsilon_F = \frac{p_F^2}{2m} = \frac{\hbar^2}{2m}\!\left(\frac{6\pi^2 n}{g}\right)^{\!2/3}
    \sim \frac{\hbar^2 n^{2/3}}{m} \equiv T_{\text{выр}}
\end{equation*}
Для электронов ($g=2$): $p_F = \hbar(3\pi^2 n)^{1/3}$, $\varepsilon_F \approx 1\div7$~эВ ($T_{\text{выр}}\sim 10^4$~К $\gg T_{\text{комн}}$). Классический режим применим при $T \gg T_{\text{выр}}$ (т.\,е. $nV_Q \ll 1$); вырождение наступает при $T \lesssim T_{\text{выр}}$.
